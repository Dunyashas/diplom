...
app.post('/api/auth/register-verify', async (req, res) => {
  ...
  let userId = req.body.userId || req.body.id || req.body.user?.id;
  const { body } = req.body;

  if (!userId) {
    return res.status(400).json({ error: 'Идентификатор пользователя (userId) не передан фронтендом или пуст' });
  }
