import { useAuth } from '../context/AuthContext';const GOLD = '#c9a84c';export default function Navbar({ onNavigate, activePage }) {
...
@media (min-width: 601px) {
  .nav-container { 
    ...
    margin: 0 auto; 
        ...
        @media (max-width: 600px) {
          ...
          .nav-container { padding: 15px 4%; height: auto; flex-direction: column; gap: 12px; }
          ...
          .nav-user-block { order: 3; width: 100%; display: flex !important; justify-content: center !important; gap: 15px !important; padding-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); }
      ...
      <div className="nav-container">
        ...
        <div className="nav-user-block" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button onClick={logout} style={{ padding: '6px 12px', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', background: 'transparent', color: '#fff', fontSize: '11px', cursor: 'pointer', letterSpacing: '1px' }}>Выйти</button>
  ...
  return (
    ...
    <button onClick={onClick} style={{ padding: '8px 16px', border: 'none', borderRadius: '4px', background: active ? 'rgba(201,168,76,0.15)' : 'transparent', color: active ? GOLD : 'rgba(255,255,255,0.7)', fontSize: '12px', cursor: 'pointer', fontFamily: 'Georgia', textTransform: 'uppercase', letterSpacing: '1px', transition: '0.2s' }}>{label}</button>
